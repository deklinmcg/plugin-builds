...
TapeTopAudioProcessorEditor::TapeTopAudioProcessorEditor (TapeTopAudioProcessor& p)
      ...
      webView (juce::WebBrowserComponent::Options{}
          .withResourceProvider (
              [this] (const juce::String& url) -> std::optional<juce::WebBrowserComponent::Resource>
              {
                  ...
                  auto html = TapeTopAudioProcessorEditor::getHTML();
...
TapeTopAudioProcessorEditor::~TapeTopAudioProcessorEditor()
{
    stopTimer();
}

void TapeTopAudioProcessorEditor::paint (juce::Graphics& g)
{
    g.fillAll (juce::Colours::black);
}

void TapeTopAudioProcessorEditor::resized()
{
    webView.setBounds (getLocalBounds());
}

void TapeTopAudioProcessorEditor::timerCallback()
{
    auto& params = audioProcessor.getParameters();
...
juce::String TapeTopAudioProcessorEditor::getHTML()
{
    return juce::String (R"HTMLEOF(
<!DOCTYPE html><html><head><meta charset="UTF-8"><style>
:root{--color-bg:#101014;--color-surface:#1A1A1F;--color-surface-hl:#292930;--color-primary:#FFC300;--color-text-primary:#F0F0F5;--color-text-secondary:#A0A0A5;--color-border:#3A3A45}
...
.value-arc{fill:none;stroke:var(--color-primary);stroke-width:6px;stroke-linecap:round}
...
<div class="plugin-header"><span>TAPETOP</span><span style="opacity:.4;font-size:8px">APC</span></div>
<div class="plugin-body">
  <div class="knob-row"><div class="knob-container" id="p0"><svg width="80" height="80" viewBox="0 0 64 64" style="overflow:visible"><path d="M12.201 51.799 A 28 28 0 1 1 51.799 51.799" fill="none" stroke="rgba(255,255,255,0.18)" stroke-width="6" stroke-linecap="round"/><circle class="value-arc" cx="32" cy="32" r="28" transform="rotate(135 32 32)"/></svg><div class="knob-value" id="v0">35%</div><div class="knob-label">DRIVE</div></div><div class="knob-container" id="p1"><svg width="80" height="80" viewBox="0 0 64 64" style="overflow:visible"><path d="M12.201 51.799 A 28 28 0 1 1 51.799 51.799" fill="none" stroke="rgba(255,255,255,0.18)" stroke-width="6" stroke-linecap="round"/><circle class="value-arc" cx="32" cy="32" r="28" transform="rotate(135 32 32)"/></svg><div class="knob-value" id="v1">120 Hz</div><div class="knob-label">HPF</div></div><div class="knob-container" id="p2"><svg width="80" height="80" viewBox="0 0 64 64" style="overflow:visible"><path d="M12.201 51.799 A 28 28 0 1 1 51.799 51.799" fill="none" stroke="rgba(255,255,255,0.18)" stroke-width="6" stroke-linecap="round"/><circle class="value-arc" cx="32" cy="32" r="28" transform="rotate(135 32 32)"/></svg><div class="knob-value" id="v2">60%</div><div class="knob-label">MIX</div></div></div>
</div>
...
window.onload=function(){_ks['p0']=Knob(document.getElementById('p0'),0,0.0,100.0,35.0,function(v){return Math.round(v)+'%';});_ks['p1']=Knob(document.getElementById('p1'),1,20.0,500.0,120.0,function(v){return v>=1000?(v/1000).toFixed(1)+'k':Math.round(v)+' Hz';});_ks['p2']=Knob(document.getElementById('p2'),2,0.0,100.0,60.0,function(v){return Math.round(v)+'%';});};
</script></body></html>
)HTMLEOF");
}
